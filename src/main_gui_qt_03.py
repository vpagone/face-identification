...
class VideoPlayer(QWidget):
    ...
    def video_shower(self, i, input_queue, fps, log_dir, label, stop_event):
        logger = get_thread_logger(log_dir, 'VideoShower')
        vs = VideoShow(i, input_queue, fps, logger, label, stop_event)
        vs.show()

    def video_recorder(self, i, input_queue, fps, duration, log_dir, out_dir):
    ...
    def start_videos(self):
        ...
        for i in range(4):
            if self.video_paths[i] != "":
                self.shower_threads[i] = threading.Thread(target=self.video_shower, args=(i, 
                                                                                          self.frame_queues_3[i],
                                                                                          self.fps[i],
                                                                                          self.log_dirs[i],
                                                                                          self.video_labels[i],
